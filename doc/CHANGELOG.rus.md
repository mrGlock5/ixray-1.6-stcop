...
## Релиз 1.3 (Октябрь 2025)

### Общее

- Реализована опция CMake для отключения Discord (@ForserX)
- Реализована поддержка UTF-8 и исправлены случаи включений (@ForserX)
- Выполнена очистка кода (@ForserX)
- Обновлен Dear ImGui до версии 1.92.1 (@GladiatorVS)
- Обновлен список пресетов для CMake (@TheParaziT)
...
- Добавлена проверка безопасности для назначения вершин назначения в методе `setup_movement_params` (@Drombeys, @ForserX)
...
- Добавлены виртуальные методы приведения для избежания dynamic_cast (@Shtrecker)
- Добавлена зависимость xrGameSpy для xrServer (@ForserX)
- Применено определение типов IX-Ray (@ForserX)
- Применен новый SDL3 в кодовой базе (@Vertver, @ForserX)
- Применена новая система сериализации для `SQLGen` (@ForserX)
- Применены виртуальные методы приведения в кодовой базе (@Shtrecker, @v2v3v4)
- Удален неиспользуемый API (@ForserX)
...
- Исправлена конвенция именования BSD для сторонних компонентов (@ForserX)
- Исправлен краш при выполнении `set_position` без звука (@St4lker0k765)
- Исправлен краш при включенном дожде с отключенными звуками (@Drombeys)
- Исправлен краш DBService и сервера во время создания пользователя (@hkuprin)
- Исправлена активация горячих клавиш с русской раскладкой (@Drombeys)
...
- Реализован (RS0) нулевой рендер для сервера (@isoboleskiy, @ForserX)
- Реализован UI `SQL Generator` (@GladiatorVS)
...
- Реализована оптимизация отправителя пакетов на основе расстояния (@ForserX)
- Реализовано экспериментальное исправление пути построения (@Drombeys)
...
- Реализована частичная функция Linux (@ForserX)
- Реализована загрузка-сохранение позиции из DBService (@hkuprin)
- Реализованы маркеры профайлера (@v2v3v4)
- Реализована поддержка AI карты и графов Shadow of Chernobyl (@St4lker0k765)
- Реализована поддержка формата архивов SoC (@St4lker0k765, @Xottab-DUTY)
- Реализован загрузочный экран с использованием фреймворка SDL (@GladiatorVS, @ForserX, @v2v3v4)
- Реализована строковая реализация стека (@wh1t3lord)
- Реализована временная оптимизация коннектора (@ForserX)
- Реализована поддержка формата YAML (@ForserX)
- Улучшена реализация выделенного сервера и FreeMP (@ForserX)
- Улучшена обработка ввода в менеджере ImGui (@Drombeys)
- Улучшен захват имен Optick (@Drombeys)
- Улучшен код безопасности сервера (@ForserX)
- Улучшена генерация SQL базы данных (@ForserX)
- Увеличено количество подэлементов игровых материалов (@Drombeys)
- Сделана SQL система опциональной (@ForserX)
- Мигрировано на Clang 18 и Ubuntu 24.04 LTS (@ForserX)
- Перемещено `XRay::Concepts` в `Concepts.h` (@Shtrecker)
- Перемещены оверлеи API в выделенную директорию (@ForserX)
...
- Рефакторинг кода платформенной файловой системы (@ForserX)
- Удалена зависимость `CXImage` (@ForserX)
- Удалена зависимость `Loki` (@ForserX)
- Удалена зависимость ATL (@ForserX)
...
- Заменена критическая секция на SRW блокировку (@v2v3v4)
...
- Переработана сохраненная игра Marshal (@Drombeys)
- Переработаны экспорты звука (@ForserX)
- Переписан `FPSCounter` с SDL3 (@ForserX)
- Установлены значения конфигурации консоли по умолчанию (@Drombeys)
- Показана текущая платформа в информации о запуске движка (@St4lker0k765)
- Обновлен класс сервера FreeMP с вызовами DBService (@hkuprin)

### Графика (рендеры, шейдеры)

- Добавлена константа `m_digiclock` (@Drombeys)
- Добавлены константы `m_timearrowN` в рендерер (@LVutner)
...
- Добавлена текстура синего шума в блендер SSLR (@LVutner)
- Добавлены шейдеры из Gunslinger (@Drombeys)
- Применена снежная маска для aref моделей (@ForserX)
- Отключена оптимизация полусфер (@Drombeys)
- Расширена функциональность G-Buffer (@Hozar2002)
...
- Исправлена реализация семплинга блоков 2x2 (@LVutner)
- Исправлена и улучшена реализация окружающего затенения (@LVutner)
- Исправлена яркость на элементах HUD (@Hozar2002)
- Исправлено изменение размера окна выделенного сервера (@ForserX)
- Исправлена яркость отображения детектора (@Drombeys)
- Исправлен некорректный расчет масштаба (@Hozar2002)
- Исправлено некорректное перечисление шейдеров и поврежденные CBuffers в compute шейдерных проходах (@LVutner)
- Исправлен расчет обратной матрицы вида (@Hozar2002)
- Исправлена шероховатость в режиме PBR terrain (@Hozar2002)
...
- Реализована обертка `Intel XESS` (@ForserX)
- Реализована базовая система PBR отражений (@Hozar2002)
- Реализована поддержка коллиматорного прицела (@Drombeys)
...
- Реализована поддержка PBR terrain (@LVutner)
- Реализована функциональность внешних шейдеров (@Hozar2002)
- Реализован проход пространственных отражений (@Hozar2002)
- Реализован TAA для D3D10+ (@LVutner)
- Реализована система сквозного прицеливания (@Hozar2002)
...
- Улучшена точность отражений (@Hozar2002)
- Улучшено качество отражений (@Hozar2002)
...
- Перереализована система отладочного рендеринга (@ugozapad)
- Удален двусторонний фильтр из шейдеров (@LVutner)
- Удалена проверка поиска свечения (@Drombeys)
...
- Разделен `IGNORE_SNOW_MASK` для террейна (@Hozar2002)
- Использован формат png для скриншотов по умолчанию (@Drombeys)

### Геймплей

- Добавлен экспорт скрипта `add_cam_effector2` (@Drombeys)
- Добавлена поддержка узла `auto_framewindow` (@St4lker0k765)
- Добавлен экспорт скрипта `CAddonManager` (@St4lker0k765)
- Добавлены экспорты скриптов `CScriptIniFile` (@Drombeys)
- Добавлены опции `DoF on Talk` в настройки (@Drombeys)
- Добавлена функция `GetDetector` (@Shtrecker)
- Добавлен экспорт скрипта `IsInCar` (@Drombeys)
- Добавлена поддержка `kQUIT` и `kSCREENSHOT` в катсценах (@ForserX)
- Добавлен атрибут `native_scale` для курсора (@St4lker0k765)
- Добавлен экспорт скрипта `set_default_visual` (@St4lker0k765, @Vyacheslav)
- Добавлен экспорт скрипта `set_fire` (@ForserX, @Vyacheslav)
- Добавлен экспорт скрипта `set_head_rotate` (@Drombeys, @Vyacheslav)
- Добавлен экспорт `set_health_ex` в Lua (@St4lker0k765)
- Добавлен экспорт скрипта `set_yaw` (@Drombeys, @Vyacheslav)
- Добавлены звуки `snd_throw` и `snd_throw_begin` для `CMissile` (@Shtrecker)
- Добавлена внешняя функция `UseSavedGameStatic` (@ForserX)
- Добавлен экспорт скрипта `world2ui` (@Drombeys)
- Добавлена возможность отключения инерции камеры от третьего лица (@St4lker0k765)
- Добавлены дополнительные звуки доставания и убирания (@Shtrecker)
- Добавлена возможность отключения сетки инвентаря (@St4lker0k765)
- Добавлены суффиксы анимаций (@Shtrecker)
- Добавлено отображение количества контейнеров артефактов брони (@Shtrecker)
- Добавлена деградация артефактов (@Shtrecker)
- Добавлены отложенные действия оружия (@Shtrecker)
- Добавлены анимации детектора с фонарем (@Shtrecker)
- Добавлена разборка предметов (@Shtrecker)
- Добавлен выход для ошибок `valid_vertex_id` (@ForserX)
- Добавлены звуки сервоприводов экзоскелета (@Drombeys)
- Добавлена внешняя функциональность для `CScriptIniFile` (@ForserX)
- Добавлен режим фреймового окна для кнопок (@St4lker0k765)
...
- Добавлен раздел последнего использования в аниматоре HUD (@Shtrecker)
- Добавлена проверка ссылок для `CleanupInvalidNodes` (@ForserX)
- Добавлен правильно типизированный метод для экспорта Lua CMapLocation (@FFx0001)
- Добавлены события чистых вызовов для реактивного хранилища (@FFx0001)
- Добавлен раздел ремонтника как третий аргумент для колбэка улучшения (@Graff46)
...
- Добавлен звук ударов дождя по шлему (@Drombeys, @v2v3v4, @VOIN101)
- Добавлен флаг спринта (@Shtrecker)
- Добавлена поддержка квадратной миникарты (@St4lker0k765)
- Добавлено статическое использование для контроля `CUI3tButton` (@Hrusteckiy)
- Добавлена опция следовых отметок для `engine_external` (@ForserX)
- Добавлены строки, связанные с бустерами (@St4lker0k765)
- Добавлено отображение бонуса дополнительного веса костюма (@Shtrecker)
- Добавлена поддержка слоистых звуков для `CCarWeapon` и `CHelicopter` (@Drombeys)
- Добавлена поддержка настраиваемого размера кнопки трекбара (@St4lker0k765)
- Добавлен перевод для максимального веса (@St4lker0k765)
- Добавлено приближение во время разговоров (@St4lker0k765)
- Применена система колбэков для вызовов скриптов из движка (@Drombeys)
- Применена величина для значений степпера и трекбара UI (@Hrusteckiy)
- Применено масштабирование величины для опций (@Drombeys)
- Предпринята попытка исправить умные укрытия Clear Sky (@Drombeys)
- Заблокирован эффектор камеры оружия во время использования детектора (@Shtrecker)
- Очищен устаревший мультиплеерный код (@St4lker0k765)
...
- Отключена функция полета в FMP (@ForserX)
- Отключен IK для выделенного сервера (@ForserX)
- Отключено отображение миникарты в инвентаре (@Drombeys)
- Отключены следовые отметки для NPC (@Shtrecker)
- Отключен спринт при перевесе (@St4lker0k765)
- Включено использование болта в FreeMP (@St4lker0k765)
- Расширена реализация звуковых эффектов фонаря (@Drombeys)
- Экспортирована функция `get_ammo_sum` в Lua (@FFx0001)
- Расширено событие `on_take` для системы колбэков (@FFx0001)
- Расширены экспорты скриптов симулятора ALife (@St4lker0k765)
- Расширены аргументы экспорта Lua для итератора CMapLocation (@FFx0001)
- Расширены экспорты XML скриптов (@Drombeys)
...
- Исправлено отображение активных контактов на миникарте в FreeMP (@ForserX)
- Исправлена реализация событий AI сталкера от клиента (@DimaKuzmin)
- Исправлен учет патронов в мультиплеере (@MiX225)
- Исправлены патроны в меню покупки TDM для Clear Sky (@St4lker0k765)
- Исправлена обработка колбэка окончания анимации (@Shtrecker)
...
- Исправлены проблемы совместимости Clear Sky и Call of Pripyat (@St4lker0k765)
- Исправлен краш после спавна игрока (@St4lker0k765)
- Исправлен краш для существующих меток карты (@St4lker0k765)
...
- Исправлен инициатор удара во время столкновений (@Graff46)
...
- Исправлено наложение при наведении на предмет (@St4lker0k765)
- Исправлено извлечение предмета из неизвестных слотов (@ForserX)
- Исправлено направление броска предмета (@MiX225)
- Исправлена функциональность журнала и энциклопедии (@St4lker0k765)
- Исправлена реализация действия огня ножа (@Shtrecker)
- Исправлена реализация удара ногой (@Shtrecker)
- Исправлена загрузка последней сохраненной игры (@Drombeys, @St4lker0k765)
- Исправлен сдвиг ног в мультиплеере (@DriveAlphinist)
- Исправлена реализация устаревшего счетчика патронов (@St4lker0k765)
- Исправлены устаревшие параметры костюма (@St4lker0k765)
- Исправлена реализация устаревшей полосы прокрутки (@St4lker0k765)
- Исправлены устаревшие проблемы UI (@St4lker0k765)
...
- Исправлен расчет переносимого веса NPC (@Drombeys, @VOIN101)
- Исправлена задержка логики NPC в мультиплеере (@Drombeys)
- Исправлены проблемы подергивания NPC (@ForserX)
- Исправлен nullptr при выпадении предмета из инвентаря (@Kuprin)
...
- Исправлены краши, связанные с указателями, и вывод условий одиночной игры (@MiX225)
...
- Исправлен тип формы `Feel_Vision` лесника Рыжего Леса (@wh1t3lord)
- Исправлены оставшиеся проблемы использования (@St4lker0k765)
- Исправлена попытка использования ремонтного набора (@Drombeys)
...
- Исправлено определение размера строки в экспортах скриптов (@Drombeys)
...
- Игнорируются отсутствующие звуки ударов `light_burn` (@St4lker0k765)
- Реализован колбэк `actor_before_death` для скрипта `bind_stalker` (@St4lker0k765)
...
- Реализован экспорт скрипта `class_info` (@wh1t3lord)
- Реализован экспорт `clone_upgrades` в Lua (@Shtrecker)
- Реализованы экспорты `CUIActorMenu` для Lua (@St4lker0k765)
- Реализована кастомизация `CUICustomSpin` (@St4lker0k765)
- Реализован контроль `CUIStackPanel` (@ForserX)
- Реализовано свойство `dest_level_name` для `cse_alife_level_changer` (@Graff46)
- Реализована консольная команда `g_hit_slowmo` для замедления при ударе по актеру (@Drombeys, @Vyacheslav)
- Реализована консольная команда `g_unlimited_durability` (@Drombeys)
- Реализован экспорт скрипта `get_actor_power_boost_time` (@Drombeys)
...
- Реализована функция скрипта `level.jump_level` (@Graff46)
- Реализован экспорт скрипта `level().main_input_receiver()` (@St4lker0k765)
- Реализован экспорт `lua_debug_print` для Lua (@Drombeys)
- Реализованы экспорты `PatrolPathStorage` для Lua (@St4lker0k765, @Drombeys)
- Реализован экспорт скрипта `r_file_as_string` (@Drombeys)
...
- Реализован экспорт скрипта `ResetOriginalRect` (@Drombeys)
- Реализован параметр `restricted_gl_and_sil` (@Shtrecker)
- Реализован экспорт скрипта `send` (@Drombeys)
- Реализован экспорт скрипта `SetARGB` (@Drombeys)
- Реализованы методы скрипта `SetFont` и `GetFont` (@St4lker0k765)
- Реализована функция `SetMisfireStatus` (@Shtrecker)
- Реализованы экспорты скрипта `SetWidth` и `SetHeight` (@St4lker0k765)
- Реализован экспорт скрипта `trigger_*` (@wh1t3lord)
- Реализован контроль `UIArrowStepper` (@Hrusteckiy)
- Реализована регулировка высоты `UIHint` на основе текстового содержимого (@Viнt@rь)
- Реализована команда 3D прицелов (@Shtrecker)
- Реализован колбэк удара актера (@St4lker0k765)
...
- Реализованы дополнительные экспорты скриптов (@St4lker0k765)
- Реализовано регулируемое масштабирование иконок (@St4lker0k765)
- Реализована продвинутая система синхронизации NPC (@ForserX)
- Реализованы анимации входа/выхода в прицел (@Shtrecker)
- Реализованы анимации движения в прицеле (@Shtrecker)
- Реализована альтернативная перезарядка для `CWeaponBM16` (@Shtrecker)
- Реализован альтернативный расчет FOV зума (@Shtrecker)
...
- Реализован флаг существования анимации скуки (@Shtrecker)
- Реализована система рюкзака (@Shtrecker, @Drombeys)
- Реализована базовая синхронизация машины (@ForserX)
...
- Реализована поддержка частей костей для оружия (@Hozar2002)
- Реализована костная система (@Shtrecker)
- Реализовано отображение времени бустеров (@Shtrecker)
- Реализован щит бюрера в мультиплеере (@MiX225)
- Реализована поддержка КПК Call of Chernobyl (@St4lker0k765)
- Реализован колбэковый подход для аномалий (@Drombeys)
- Реализован звук смены типа патронов (@Shtrecker)
- Реализована система патрона в патроннике (@Shtrecker)
- Реализовано действие очистки маски (@Shtrecker)
- Реализован слой совместимости базового HUD Clear Sky (@St4lker0k765)
- Реализована поддержка коллиматорного прицела (@Shtrecker)
- Реализовано выделение разделов предметов через запятую (@FFx0001)
- Реализованы условия для сменщиков уровней (@Graff46)
- Реализованы параметры контактной гранаты (@Shtrecker)
...
- Реализован настраиваемый размер изображения сохраненной игры (@St4lker0k765)
- Реализована кастомизация контроля `CUIComboBox` (@St4lker0k765)
- Реализована отложенная смена рук (@Shtrecker)
- Реализованы анимации броска болта детектора (@Shtrecker)
- Реализована функциональность принудительного скрытия детектора (@Shtrecker)
- Реализованы анимации доставания и убирания рук детектора (@Shtrecker)
- Реализованы улучшения детектора в мультиплеере (@MiX225)
- Реализованы анимации удара ногой детектора (@Shtrecker)
...
- Реализованы DoF и увеличение FOV во время диалога NPC (@Drombeys, @OldSerpskiStalker)
- Реализована функциональность колбэка дверей (@St4lker0k765)
- Реализована функциональность дозиметра (@ForserX)
- Реализованы контроли гаммы и контрастности шейдеров DX11 (@ForserX, @Hozar2002, @RainbowZerg)
- Реализован динамически расширяемый пояс артефактов (@Drombeys)
- Реализована функция проблем с электроникой (@Shtrecker)
- Реализованы анимации пустого и заклинившего клика (@Shtrecker)
- Реализовано выделение экипированного предмета (@St4lker0k765)
...
- Реализована поддержка анимаций дробовика мода Gunslinger (@Shtrecker)
...
- Реализован колбэк удара для актеров (@St4lker0k765)
- Реализованы ограничения источника удара (@Graff46)
- Реализована функциональность горизонтального спина (@St4lker0k765)
- Реализована функциональность обновления раздела HUD (@Shtrecker, @Drombeys)
- Реализована кастомизация цели HUD (@St4lker0k765)
- Реализована база света фонаря HUD (@Shtrecker)
...
- Реализовано сохранение статуса инвентаря и КПК (@St4lker0k765)
...
- Реализовано переопределение нажатия клавиш скриптом (@St4lker0k765)
- Реализованы звуковые эффекты удара ногой (@Shtrecker, @Drombeys)
- Реализованы слоистые звуки только для стрельбы (@Shtrecker)
- Реализованы слоистые звуки для стационарных пулеметов (@Drombeys)
- Реализована поддержка устаревшего XML `actor_menu_item` (@St4lker0k765)
- Реализована поддержка устаревшего `inventory_upgrade.xml` (@St4lker0k765)
- Реализована поддержка устаревшего КПК (@St4lker0k765)
- Реализован перевод названий уровней для скриншотов (@St4lker0k765)
- Реализована облегченная схема костей патронов (@Shtrecker)
- Реализованы расширения таблиц и строк Lua (@Drombeys)
- Реализовано механическое свойство для клиентских NPC через скрипты (@Graff46)
- Реализовано переключение режима осечек (@St4lker0k765)
- Реализована поддержка смешанного `level.ai` для построения спавна (@ForserX)
- Реализован генератор суффиксов движений (@Shtrecker)
- Реализовано воспроизведение звука на основе движения (@Shtrecker)
...
- Реализована фокусировка на лице NPC во время диалога (@Drombeys)
- Реализована конфигурация голоса NPC (@Graff46)
- Реализован обходной путь использования оружия NPC (@Drombeys)
...
- Реализована опциональная перестановка кнопок КПК (@St4lker0k765)
- Реализован опциональный слот пистолета в инвентаре (@St4lker0k765)
- Реализовано опциональное отображение иконки прицела за оружием (@Drombeys, @Hozar2002, @VOIN101)
- Реализован опциональный фактор FOV спринта (@Drombeys)
- Реализована частичная функция из issue 2055 (@ForserX, @Vyacheslav)
- Реализована частичная мультиплеерная функция (@ForserX)
...
- Реализована правильная проверка круглой карты (@St4lker0k765)
- Реализованы свойства из Call of Chernobyl (@St4lker0k765)
- Реализован звуковой эффект помпового механизма (@Shtrecker)
- Реализованы чистые колбэки инвентарного ящика (@FFx0001)
...
- Реализован экспорт скрипта raypick (@St4lker0k765)
...
- Реализованы безопасные обновления камеры игрока в транспорте (@ForserX)
- Реализована функциональность расширения сохранений (@St4lker0k765, @Drombeys, @wh1t3lord)
- Реализовано масштабирование расписания в выделенном сервере (@DimaKuzmin)
- Реализована возможность создания скриншотов с отключенным вводом (@St4lker0k765)
- Реализованы события и API скриптов из OMP (@ForserX)
- Реализованы экспорты скриптов из issue 2055 (@ForserX, @Vyacheslav)
- Реализована функциональность чтения папки скриптов (@DimaKuzmin)
- Реализовано отключаемое через скрипты КПК и инвентарь (@St4lker0k765)
- Реализовано создание форм в контекстном меню (@ForserX)
...
- Реализованы анимации входа/выхода в спринт (@Shtrecker)
...
- Реализовано отображение информации об улучшениях в двух колонках (@Hrusteckiy, @Drombeys)
...
- Реализована функция быстрого удара ногой оружием (@Shtrecker)
- Реализована разгрузка оружия и подключение пушечного модуля (@MiX225)
- Улучшена функция `getAmmoTypes` (@Shtrecker)
- Улучшена система ИИ (@Drombeys)
- Улучшена система выделения (@St4lker0k765)
...
- Улучшена система рейтинга UI (@St4lker0k765)
- Улучшена совместимость с ультраширокими экранами (@St4lker0k765)
- Улучшена обработка состояния кнопки улучшения (@Shtrecker)
- Сделан шаг настройки HUD зависимым от чувствительности мыши (@v2v3v4)
- Сделан селектор скинов менее зависимым от платформы (@St4lker0k765)
- Перемещены конфигурации backend UI в отдельную папку (@Hrusteckiy)
- Перемещен GameSP в реализацию CurrentGameUI (@DimaKuzmin)
...
- Портирована система сервера из Clear Sky (@St4lker0k765)
- Предотвращено появление сумки при отключении (@ForserX)
...
- Рефакторинг воспроизведения звука и реализованы звуковые флаги (@Shtrecker)
- Рефакторинг UIWpnParams и исправлены проблемы границ массивов (@v2v3v4)
...
- Удалена валидация CD-Key (@St4lker0k765)
- Удален краш невалидной вершины (@St4lker0k765)
- Удален обходной путь устаревшего счетчика патронов (@St4lker0k765)
- Удалены проверки платформы из модуля `UISkinSelector` (@St4lker0k765)
- Заменены серверы GameSpy на серверы GSC (@St4lker0k765)
- Заменены тире в тексте на символ бесконечности в игре (@Drombeys)
- Восстановлен экспорт скрипта `alife_simulator:object` (@Drombeys)
...
- Восстановлено окно фракционной войны в КПК (@St4lker0k765)
- Восстановлен устаревший счетчик патронов из Clear Sky (@St4lker0k765)
- Восстановлены устаревшие управления из Clear Sky (@St4lker0k765)
- Восстановлены устаревшие экспорты скриптов (@Drombeys)
- Восстановлена функциональность мутантов (@St4lker0k765)
- Восстановлено касание сумки игрока в FreeMP (@ForserX)
- Восстановлено выделение текста в стиле SoC (@St4lker0k765)
...
- Добавлен xml `ui_actor_sleep_screen` в Clear Sky (@St4lker0k765)
...
- Добавлен выбор пункта диалогового меню по номеру (@Drombeys)
...
- Добавлены отсутствующие мультиплеерные файлы в Clear Sky (@St4lker0k765)
...
- Добавлен эффект сытости на здоровье (@Drombeys)
...
- Портирован скрипт `level_weather` в Clear Sky (@Drombeys)
- Портированы скрипты `pda` и `bind_stalker` в Clear Sky (@St4lker0k765)
- Портирован скрипт `ph_door` в Clear Sky (@Drombeys)
- Портирован скрипт `sr_no_weapon` в Clear Sky (@Drombeys)
- Портирован xml `ui_detector_artefact` в Clear Sky (@St4lker0k765)
- Портирован скрипт `xr_sound` в Clear Sky (@Drombeys)
- Портированы и применены новые опции в Clear Sky (@St4lker0k765)
- Портирована функция скидок для торговли в Clear Sky (@Hrusteckiy)
- Портирован нож в Clear Sky (@Shtrecker)
- Портированы диалоги загрузки и сохранения в Clear Sky (@St4lker0k765)
- Портирован диалог загрузки в Clear Sky (@St4lker0k765)
- Портирована система постпроцесса в Clear Sky (@Drombeys)
- Портированы конфигурации прицелов в Clear Sky (@Shtrecker)
- Обойден краш на невалидной AI карте в точке кампы в Clear Sky (@Drombeys)
...
- Исправлен xml `car_panel` в Clear Sky (@St4lker0k765)
...
- Исправлены 6 сокровищ Янтаря от 5 сталкеров из Агропрома в Clear Sky (@K0cTep)
...
- Исправлено столкновение прыжка кровососа в Clear Sky (@wh1t3lord)
- Исправлены проблемы крашей Clear Sky (@St4lker0k765)
- Исправлен размер стрелки комбо-бокса на широком экране (@St4lker0k765, @Viнt@rь)
- Исправлено название сообщества в окне разговора в Clear Sky (@Hrusteckiy)
- Исправлены иконки контроллера в Clear Sky (@St4lker0k765)
- Исправлены пользовательские статики в Clear Sky (@St4lker0k765)
- Исправлено движение пояса артефактов по умолчанию в Clear Sky (@St4lker0k765)
- Исправлены проверки диалога для оружия (@Hozar2002, @Drombeys)
- Исправлена фраза диалога для исправителя Свободы (@K0cTep)
...
- Исправлено складывание гранат в инвентаре в Clear Sky (@St4lker0k765)
- Исправлены проверки гида и bind_signal_light в Clear Sky (@Viнt@rь)
...
- Исправлены зацикленные награды Винтореза и артефакта в Clear Sky (@K0cTep)
...
- Исправлены мультиплеерные скрипты в Clear Sky (@St4lker0k765)
- Исправлен мультиплеерный UI в Clear Sky (@St4lker0k765)
...
- Исправлена синхронизация времени сохраненной игры в Clear Sky (@wh1t3lord)
...
- Исправлена перезарядка звука оружия `wpn_pkm` в Clear Sky (@K0cTep)
- Исправлена перезарядка звука оружий `wpn_spas12` и `wpn_wincheaster1300` в Clear Sky (@Drombeys)
- Исправлена перезарядка звука оружия `wpn_svd` в Clear Sky (@Drombeys)
...
- Исправлено сокровище для лидера Долга в Clear Sky (@K0cTep)
- Исправлены элементы UI в Clear Sky (@St4lker0k765)
- Исправлены иконки улучшений на широком экране в Clear Sky (@Hrusteckiy)
- Исправлены различные проблемы UI (@St4lker0k765)
- Исправлена иконка Васяна в сообщении поиска трупа в Clear Sky (@K0cTep)
...
- Реализована настройка скорости шниаги (@St4lker0k765)
- Реализован обходной путь одноцветной кнопки (@St4lker0k765)
- Реализован параметр snd_bore для оружия в Clear Sky (@Drombeys)
- Реализована поддержка улучшений для Clear Sky (@St4lker0k765)
- Реализована рабочая смертельная схватка в Clear Sky (@St4lker0k765)
- Улучшено поведение фонаря в бою (@Drombeys)
...
- Сделаны бандиты в засаде неуязвимыми в Clear Sky (@K0cTep)
- Перемещены конфигурации backend UI в собственную папку (@Drombeys)
...
- Удален `xrRender_DS0` из списка рендеров (@St4lker0k765)
- Удалена возможность входа в Темную долину до завершения сюжетных миссий в Clear Sky (@K0cTep)
- Удален мертвый код из скриптов (@Drombeys)
- Удалены временные иконки заданий в Clear Sky (@Drombeys)
...
- Восстановлены пресеты звуковой среды для Shader Editor (@ForserX)
...
- Добавлена поддержка `user.ltx` в Level Editor (@St4lker0k765)
- Добавлена вкладка аномалий в Spawn Manager (@Drombeys)
- Добавлена вкладка устройств для Spawn Manager (@Drombeys)
- Добавлена вкладка динамических объектов в Spawn Manager (@Drombeys)
- Добавлен рендер динамических частиц в Level Editor с `Edit Mode` (@ForserX)
- Добавлена информация Embree (@Dimakuzmin)
- Добавлена вкладка взрывчатки в Spawn Manager (@Drombeys)
- Добавлены FontAwesome и иконки глаза в Edit Mode (@GladiatorVS)
- Добавлена пользовательская конфигурация Level Editor (@ForserX)
- Добавлены отсутствующие классы предметов в Spawn Manager (@Drombeys)
- Добавлена вкладка монстров в Spawn Manager (@Drombeys)
- Добавлены новые параметры для xrAI и xrDO (@DimaKuzmin)
- Добавлена вкладка NPC в Spawn Manager (@Drombeys)
...
- Добавлена вкладка отрядов в Spawn Manager (@Drombeys)
- Добавлена поддержка `Snap List` для `CTerrain` (@ForserX)
- Добавлен поиск по переведенным именам в Spawn Manager (@Drombeys)
- Скорректированы пропорции загрузочного экрана (@GladiatorVS)
- Скорректировано расстояние Tnear и рефакторинг связанного кода (@DimaKuzmin)
- Попытка исправить рендеринг деталей в Редакторе Уровней (@ForserX)
- Изменен `ResolveMaterials` на многопоточность (@Dimakuzmin)
- Изменен цвет иконки глаза и визуальные состояния (@GladiatorVS)
- Улучшена реализация фильтра (@Dimakuzmin)
- Улучшен код TBB MergeGeom (@Dimakuzmin)
- Улучшен код xrLevelBuilder (@Dimakuzmin)
- Полностью переработана система компилятора (@DimaKuzmin)
- Отключена проверка сущностей для Debug сборок (@ForserX)
- Отключен сменщик уровня в Режиме Игры в Редакторе (@ForserX)
- Отключено освещение для целей тестирования (@Dimakuzmin)
- Отключена строка поиска в элементе группы редактирования и в Редакторе Библиотеки (@ForserX)
- Включена функциональность Build.cform (@Dimakuzmin)
- Расширен вывод информации о стадии (@DimaKuzmin)
...
- Исправлен вызов `Clear` в Режиме Игры в Редакторе (@ForserX)
- Исправлено действие `delete` в `Браузере Контента` (@ForserX)
...
- Исправлено использование многопоточности в `NvTriStrip` (@Dimakuzmin)
...
- Исправлена работа с параметрами кнопок Axis и Old Camera (@xr-Renn)
- Исправлена Batch Convert с новыми файлами (@ForserX)
- Исправлено граничное окно в редакторах (@ForserX)
- Исправлен расчет ограничивающей рамки без `CSceneObject` (@ForserX)
- Исправлено использование памяти CompilerUI с обновлениями раз в 2 секунды (@Dimakuzmin)
- Исправлена функциональность Браузера Контента (@GladiatorVS)
- Исправлены описания в режиме списка Браузера Контента (@ForserX)
- Исправлен рендеринг списка объектов деталей (@ForserX)
- Исправлена загрузка деталей (detail objects) (@ForserX)
- Исправлены события перетаскивания в UI (@ForserX)
- Исправлено перетаскивание в Браузере Контента (@ForserX)
- Исправлена ошибка Embree при длительной компиляции без текстур (@Dimakuzmin)
...
- Исправлено исключение неверного начального каталога в Браузере Контента (@ForserX)
- Исправлено падение Редактора Уровней в секторах при `Reload Object` (@ForserX)
- Исправлены предупреждения компоновщика в Редакторе Частиц (@ForserX)
- Исправлены проблемы повреждения памяти в компиляторе (@ForserX)
...
- Исправлено вращение объектов в Редакторе Уровней (@ForserX)
- Исправлено дополнительное освещение для MU-моделей (@ForserX)
- Исправлена функциональность Режима Игры в Редакторе (@ForserX)
- Исправлена функциональность выхода в Редакторе Актеров (@ForserX)
- Исправлен звук дождя в Режиме Игры в Редакторе (@ForserX)
- Исправлен рендеринг объектов без уровня (@ForserX)
- Исправлено масштабирование с вращением в Редакторе Уровней (@ForserX)
- Исправлено построение секторов и геометрии в xrLevelBuilder (@Dimakuzmin)
- Исправлена реализация сектора и удалена ненужная многопоточность (@DimaKuzmin)
- Исправлено выделение в других видовых окнах (@ForserX)
- Исправлен рендеринг звуковых окружений в Редакторе Шейдеров (@ForserX)
- Исправлены проблемы переполнения стека в утилитах (@ForserX)
- Исправлено странное падение `MakeProgressive` (@Dimakuzmin)
- Исправлена обработка расширений файлов тем (@xr-Renn)
- Исправлена блокировка потока при удалении групповых объектов (@ForserX)
- Исправлены иконки верхней панели и кнопки Браузера Контента (@GladiatorVS)
- Исправлены типы и функции в xrLevelBuilder (@ForserX)
- Исправлена реализация выбора иконок в UI (@GladiatorVS)
- Исправлена функциональность кнопок Отмена и Повтор (@ForserX)
- Исправлены кнопки Отмена и Повтор (@xr-Renn)
- Исправлена обработка UTF-8 в xrLevelBuilder (@ForserX)
- Исправлены проблемы реализации утилит (@ForserX)
- Исправлено поколение UV-карты для карт высот (@ForserX)
- Исправлена функциональность видовых окон в других редакторах (@ForserX)
- Исправлен рендеринг отметок на стенах в Редакторе Уровней (@ForserX)
- Исправлена строка окна в редакторах (@ForserX)
- Исправлены названия параметров прозрачности окна в редакторе тем (@xr-Renn)
...
- Реализован выборщик `Базовой Текстуры` в `Инструментах Объекта` (@ForserX)
...
- Реализована поддержка 25-битных AI-Карт (@ForserX, @abramcumner)
- Реализована поддержка 26-битных AI-Карт (@SkyLoaderr)
- Реализованы дополнительные функции объектов terrain (@ForserX)
- Реализованы сохранение/загрузка AI-Карты и Деталей в Редакторе Уровней (@rederr404)
- Реализована навигация настроек якоря в xrForms (@GladiatorVS)
...
- Реализован базовый код для OMF Editor (@wh1t3lord)
...
- Реализована поддержка сборки terrain (@ForserX)
- Реализован clipper для Лога Редактора (@GladiatorVS)
...
- Реализована система пользовательских атласов иконок (@Drombeys, @St4lker0k765)
- Реализовано создание диалогов для Редактора Диалогов (@ForserX)
- Реализована поддержка каталогов для Python плагинов (@ForserX)
- Реализовано подсвечивание при перетаскивании (@ForserX)
- Реализовано направление камеры редактора в Режиме Игры в Редакторе (@ForserX)
- Реализована быстрая проверка `FindDuplicateName` (@ForserX)
- Реализована настройка точек выстрела и гильз (@St4lker0k765)
...
- Реализован экспорт устаревших AI-Карт для Редактора Уровней (@ForserX)
- Реализовано закрытие уровня в xrLC после завершения записи (@DimaKuzmin)
- Реализована поддержка Lmaps с разрешением от 1k до 8k (@DimaKuzmin)
- Реализован матричный режим для манипулятора в Редакторе Уровней (@ForserX)
- Реализовано видовое окно для мешей (@ForserX)
- Реализован перемещаемый terrain (@ForserX)
- Реализованы иконки множественного выбора в Браузере Контента (@GladiatorVS)
- Реализована поддержка множественного выбора в Редакторе Изображений (@ForserX)
...
- Реализовано дополнительное окно свойств для `Редактора Библиотеки` (@ForserX)
- Реализована частичная функциональность Редактора Диалогов (@ForserX)
- Реализовано видовое окно для частиц (@ForserX)
- Реализовано правильное обнуление указателей (@ForserX)
- Реализовано быстрое открытие файлов через Браузер Контента (@ForserX)
- Реализована опция отключения столкновений дождя в Редакторе Уровней (@Romann)
- Реализован экспорт Rcast моделей для Intel Embree (@DimaKuzmin)
- Реализован красный цвет для несохраненных названий уровней (@ForserX)
- Реализована кнопка сброса в Редакторе Погоды (@GladiatorVS)
- Реализован инструмент сцены для карт высот (@ForserX)
- Реализован поиск для `UIPropertiesForm` (@ForserX)
- Реализован поиск для Редактора Библиотеки (@ForserX)
...
- Реализован просмотр поверхностей в `Редакторе Библиотеки` (@ForserX)
...
- Реализована настройка потоков и исправлены проблемы с вершинами Embree (@Dimakuzmin)
- Реализовано размещение ванильных DDS с поддержкой многопоточности (@Dimakuzmin)
- Реализовано освещение на основе погоды в Редакторе Уровней (@ForserX)
- Реализован обходной путь для Режима Игры в Редакторе (@ForserX)
- Реализован обходной путь для падения SE в FillProp (@ForserX)
- Реализован UI для xrCompress (@GladiatorVS)
- Улучшена функциональность `UIEditLibrary` (@ForserX)
- Улучшена и оптимизирована проверка сцены (@ForserX)
- Улучшена реализация компилятора (@DimaKuzmin)
- Улучшен UI компилятора (@GladiatorVS)
- Улучшено объединение геометрии (@DimaKuzmin)
- Улучшены компиляторы уровней (@ForserX)
- Улучшена система соединения уровней между LE и xrAI (@ForserX)
- Улучшена фильтрация проектов в Редакторе Уровней (@ForserX)
- Улучшен пользовательский интерфейс Редактора Уровней (@ForserX)
- Улучшена подсистема Lua плагинов (@ForserX)
- Улучшен OMF Editor (@wh1t3lord)
...
- Увеличена емкость подкарт уровня (@Romann)
...
- Оптимизировано объединение геометрии в xrLC (@DimaKuzmin)
...
- Предотвращено падение Режима Игры в Редакторе без shapes (@ForserX)
- Переработана логика браузера контента (@GladiatorVS)
...
- Рефакторинг реализации шаблонов потоков (@ForserX)
- Удалены DumpFlags из класса device (@ForserX)
...
- Удалена устаревшая панель инструментов из Редактора Актеров (@St4lker0k765)
- Удалены устаревшие кнопки в Редакторе Актеров (@ForserX)
- Удалены устаревшие проверки ifdef (@ForserX)
- Удалены старые события Borland (@ForserX)
- Заменен глобальный `CDB::MODEL` на умный указатель в xrAI (@ForserX)
- Заменен код мешей d3dx9 на DirectXMesh (@ForserX)
- Заменен blob Embree на пакет NuGet (@ForserX)
- Заменены флаги таблиц ImGui в редакторах (@Romann)
- Заменен QSlim на пользовательскую реализацию MxQuadric (@ForserX)
- Заменены иконки видовых окон на FontAwesome (@GladiatorVS)
...
- Восстановлено отображение названия уровня в верхней панели (@ForserX)
- Восстановлен метод размещения LM в DDS файлах (@Dimakuzmin)
- Восстановлено отображение подсказок мыши в Редакторе Уровней (@ForserX)
...
- Переписан выбор текстур в Редакторе Погоды (@GladiatorVS)
...
- Стабилизирована система анимации (@ForserX)
...
- Обновлена реализация UV и объединения геометрии (@DimaKuzmin)
