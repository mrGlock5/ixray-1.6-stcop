...
- Applied IX-Ray Types definition (@ForserX)
- Applied new SDL3 to codebase (@Vertver, @ForserX)
- Applied new serialization system to `SQLGen` (@ForserX)
...
- Fixed DBService and server crash during user creation (@hkuprin)
...
- Implemented position save-load from DBService (@hkuprin)
- Implemented profiler markers (@v2v3v4)
...
- Moved `XRay::Concepts` to `Concepts.h` (@Shtrecker)
- Moved API overlays to dedicated directory (@ForserX)
...
- Replaced critical section with SRW lock (@v2v3v4)
...
- Updated FreeMP server class with DBService calls (@hkuprin)

### Graphics (renders, shaders)

- Added `m_digiclock` constant (@Drombeys)
- Added `m_timearrowN` constants to renderer (@LVutner)
...
- Added blue noise texture to SSLR blender (@LVutner)
...
- Fixed dedicated server window resizing (@ForserX)
...
- Implemented `Intel XESS` wrapper (@ForserX)
...
- Improved reflection accuracy (@Hozar2002)
- Improved reflections quality (@Hozar2002)
...
- Removed glow finding check (@Drombeys)
...
- Splitted `IGNORE_SNOW_MASK` for terrain (@Hozar2002)
...
- Added frame window mode for buttons (@St4lker0k765)
...
- Added last use section to HUD animator (@Shtrecker)
...
- Added repairman section as third argument to upgrade callback (@Graff46)
...
- Added zoom-in during conversations (@St4lker0k765)
...
- Attempted to fix Clear Sky smart covers (@Drombeys)
...
- Enabled bolt usage in FreeMP (@St4lker0k765)
...
- Extended Lua export arguments for CMapLocation iterator (@FFx0001)
...
- Fixed Clear Sky and Call of Pripyat compatibility issues (@St4lker0k765)
- Fixed crash after player spawn (@St4lker0k765)
- Fixed crash for existing map locations (@St4lker0k765)
...
- Fixed door implementation for FMP (@ForserX)
...
- Fixed knife fire action implementation (@Shtrecker)
...
- Fixed remaining usage issues (@St4lker0k765)
...
- Implemented 3D scopes command (@Shtrecker)
...
- Implemented NPC face focus during dialogue (@Drombeys)
...
- Fixed sound reload of `wpn_pkm` weapon in Clear Sky (@K0cTep)
- Fixed sound reload of `wpn_spas12` and `wpn_wincheaster1300` weapons in Clear Sky (@Drombeys)
- Fixed sound reload of `wpn_svd` weapon in Clear Sky (@Drombeys)
